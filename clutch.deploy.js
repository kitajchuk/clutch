...
if ( process.env.NODE_ENV === "staging" ) {
    ...
    child_process.execSync( `ssh ${process.env.AWS_USER}@${process.env.AWS_STAGING_HOST} 'cd ${process.env.AWS_DEST}server/ ; sudo echo ${Date.now()} > clutch.timestap ; sudo npm run install:staging'` );
    ...
    child_process.execSync( `ssh ${process.env.AWS_USER}@${process.env.AWS_PRODUCTION_HOST} 'cd ${process.env.AWS_DEST}server/ ; sudo echo ${Date.now()} > clutch.timestap ; sudo npm run install:staging'` );
}
