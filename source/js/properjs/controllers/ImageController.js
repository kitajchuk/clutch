...
class ImageController extends Controller {
    constructor ( images, handler ) {
        super();

        this.loaders = {};
        this.handler = handler || util.isElementVisible;

        // Are we nooping the entire batch?
        if ( this.handler === util.noop ) {
            this.preload = images;
            this.lazyload = [];
            log( `ImageController::noop`, images.length );
        ...
        if ( this.lazyload.length ) {
            this.handleLoading( this.lazyload, "lazyload", this.handler );
        }
    }
    ...
    handleLoading ( images, event, handler ) {
        ...
        this.loaders[ event ].on( "done", () => {
            log( `ImageController::${event}-done`, images.length );
